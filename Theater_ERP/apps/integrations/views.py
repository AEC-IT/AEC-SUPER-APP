...
class DistrictDCRReportViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['status', 'mismatch_flag', 'review_status', 'posting_status', 'report_date']
    ...
    def reprocess(self, request, pk=None):
        ...
        try:
            ...
            report.parsed_convenience_fee = parsed_data.get('parsed_convenience_fee', Decimal('0'))
            report.parsed_repbeta = parsed_data.get('repbeta', Decimal('0'))
            report.parsed_kfc = parsed_data.get('kfc', Decimal('0'))
            ...
            if abs(computed_nett - report.parsed_nett_revenue) > DCRValidator.TOLERANCE:
                ...
                DCRDiscrepancy.objects.create(
                    ...
                    description=f"Nett math mismatch: expected (Gross-Expenses) {computed_nett}, parsed {report.parsed_nett_revenue}",
            ...
            report.save()
            DCRValidator.sync_to_movies_and_bookings(report)
