...
from apps.tenants.mixins import TenantQuerysetMixin, get_tenant_from_request
...
class BookingViewSet(TenantQuerysetMixin, viewsets.ModelViewSet):
    ...
    - Queryset is automatically scoped to request.tenant via TenantQuerysetMixin.
    - Public create (AllowAny) stamps tenant from show.screen.tenant via BookingService.
    """
    queryset = Booking.objects.select_related(
        'show__movie', 'show__screen', 'tenant'
    ).order_by('-created_at')
