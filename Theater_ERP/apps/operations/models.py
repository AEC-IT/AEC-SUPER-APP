...
class TenantAsset(models.Model):
    ...
    is_active             = models.BooleanField(default=True)
    counts_down           = models.BooleanField(
        default=False,
        help_text="If True, the reading decreases over time (e.g. 3000→2999). Delta is treated as absolute consumption."
    )
    ...
    def life_percentage(self):
        if self.template.rated_life_hours and self.template.rated_life_hours > 0:
            ...
            if remaining is None:
                return None
            return (remaining / self.template.rated_life_hours) * 100
        return None
...
class AssetLog(models.Model):
    ...
    def save(self, *args, **kwargs):
        raw_delta = self.closing_value - self.opening_value
        # For countdown lamps opening > closing, delta is negative, but consumption is |delta|
        self.delta = abs(raw_delta)
        super().save(*args, **kwargs)
