...
class TenantAssetSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'installed_date', 'current_hours', 'alert_threshold_hours', 'life_percentage',
            'remaining_hours', 'is_active', 'counts_down'
        ]
...
class AssetLogSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # For countdown lamps (3000→2999), closing < opening is expected — skip direction check
        counts_down = getattr(asset, 'counts_down', False) if asset else False
        if not counts_down and opening is not None and closing is not None and closing < opening:
        ...
        if asset and asset.template.rated_life_hours and asset.template.category.tracks_hours:
            delta = abs(closing - opening) if (opening is not None and closing is not None) else 0
            if asset.remaining_hours is not None and (asset.remaining_hours - delta) < 0 and not data.get('override'):
                raise serializers.ValidationError({
...
class TenantAssetViewSet(TenantSafeMixin, viewsets.ModelViewSet):
    ...
    filterset_fields = ['is_active', 'template__category', 'template__category__key', 'screen', 'status', 'asset_id']
...
class AssetLogViewSet(TenantAuditMixin, viewsets.ModelViewSet):
    ...
    ordering = ['log_date']   # default ascending
    permission_classes = [IsStaffOrAbove]
...
class TheaterDashboardViewSet(viewsets.ViewSet):
    permission_classes = [IsAuthenticated]

    def list(self, request):
        return Response({"message": "Theater Dashboard API"})
    ...
    def summary(self, request):
        ...
        if tenant:
            booking_qs = booking_qs.filter(tenant=tenant)
            
        confirmed_bookings_count = booking_qs.count()
        
        # 2. Revenue (confirmed bookings total_amount in current month)
        monthly_revenue_qs = booking_qs.filter(created_at__date__gte=first_of_month)
        from django.db.models import Sum
        monthly_revenue = monthly_revenue_qs.aggregate(total=Sum('total_amount'))['total'] or 0.0
        
        # 3. Fault tickets (Open, In Progress, Escalated)
        ticket_qs = FaultTicket.objects.filter(
            status__in=[FaultTicket.Status.OPEN, FaultTicket.Status.IN_PROGRESS, FaultTicket.Status.ESCALATED]
        )
        if tenant:
            ticket_qs = ticket_qs.filter(tenant=tenant)
        open_tickets = ticket_qs.count()
        
        # 4. PM Schedules (Active next due date)
        pm_qs = PMSchedule.objects.filter(status=PMSchedule.Status.ACTIVE)
        if tenant:
            pm_qs = pm_qs.filter(tenant=tenant)
        active_pm_schedules = pm_qs.count()
        
        # 5. Last 7 days daily revenue
        daily_revenue = []
        for i in range(7):
            day = today - datetime.timedelta(days=i)
            day_rev = booking_qs.filter(created_at__date=day).aggregate(total=Sum('total_amount'))['total'] or 0.0
        ...
        # 6. Last 6 months monthly revenue history
        ...
        for i in range(6):
            ...
            month_rev = booking_qs.filter(created_at__date__gte=month_start, created_at__date__lt=next_month_start).aggregate(total=Sum('total_amount'))['total'] or 0.0
            monthly_revenue_history.append({
                'month': month_start.strftime('%Y-%m'),
                'revenue': float(month_rev)
            })
        monthly_revenue_history.reverse()
        
        return Response({
            'confirmed_bookings': confirmed_bookings_count,
            'monthly_revenue': float(monthly_revenue),
            'open_tickets': open_tickets,
            'active_pm_schedules': active_pm_schedules,
