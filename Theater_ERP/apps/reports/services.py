...
class PLReportEngine:
    ...
    def get_daily_report(report_date: date, tenant=None) -> dict:
        ...
        if tenant:
            enabled_modules = set(
                TenantModule.objects.filter(tenant=tenant, is_enabled=True).values_list('module_key', flat=True)
            )
        else:
            # If no tenant context, default to all modules active
            enabled_modules = {
                'OPERATIONS', 'LAMP_REGISTRY', 'SCREEN_BUILDER', 'PRICING_ENGINE',
                'CAFE', 'ADVERTISING', 'FINANCE', 'MD_DASHBOARD', 'AUDIT', 'BOOKINGS',
                'PAYROLL_MIRROR', 'PETPOOJA'
            }
        ...
        bookings_count = 0
        tickets_sold = 0
        if 'BOOKINGS' in enabled_modules:
        ...
        canteen_qs = None
        canteen_gross = Decimal('0')
        cafe_exp_qs = None
        cafe_expense = Decimal('0')
        if 'CAFE' in enabled_modules:
        ...
        # Canteen revenue is gross concession sales.
        # Cafe expense is listed under expenses to avoid double-deductions.
        canteen_revenue = canteen_gross

        ad_revenue = Decimal('0')
        if 'ADVERTISING' in enabled_modules:
        ...
        if 'OPERATIONS' in enabled_modules:
            ...
            if tenant: elec_qs = elec_qs.filter(meter__tenant=tenant)
        ...
        distributor_cost = Decimal('0')
        if 'FINANCE' in enabled_modules:
        ...
        daily_payroll = Decimal('0')
        if 'PAYROLL_MIRROR' in enabled_modules:
            ...
            if tenant: pay_qs = pay_qs.filter(staff__tenant=tenant)
        ...
        # ── DCR Levies (GST, ETax, CESS, RepBeta, KFC) from District DCR ──────
        dcr_qs = DistrictDCRReport.objects.filter(report_date=report_date)
        if tenant: dcr_qs = dcr_qs.filter(tenant=tenant)
        dcr_agg = dcr_qs.aggregate(
            gst=Sum('parsed_gst'),
            etax=Sum('parsed_etax'),
        ...
        dcr_cess     = dcr_agg['cess']     or Decimal('0')
        ...
        cafe_units_summary = []
        if 'CAFE' in enabled_modules and canteen_qs is not None and cafe_exp_qs is not None:
        ...
        # ── ALERTS ────────────────────────────────────────────────────
        alerts = []
        if 'BOOKINGS' in enabled_modules and 'OPERATIONS' in enabled_modules:
        ...
        return {
            ...
            'expenses': {
                ...
                'dcr_gst': float(dcr_gst),
                'dcr_etax': float(dcr_etax),
                'dcr_cess': float(dcr_cess),
                'dcr_repbeta': float(dcr_repbeta),
                'dcr_kfc': float(dcr_kfc),
                'dcr_levies_total': float(dcr_levies_total),
    ...
    def get_monthly_report(month: int, year: int, tenant=None) -> dict:
        ...
        # ── DCR Levies (GST, ETax, CESS, RepBeta, KFC) from District DCR ──────
        dcr_qs = DistrictDCRReport.objects.filter(report_date__range=[start, end])
        if tenant: dcr_qs = dcr_qs.filter(tenant=tenant)
        dcr_agg = dcr_qs.aggregate(
            gst=Sum('parsed_gst'),
            etax=Sum('parsed_etax'),
    ...
    def drill_down_source(period_start: date, period_end: date, source_module: str, category: str = None, tenant=None) -> list:
        ...
        if source_module == 'BOOKINGS' and 'BOOKINGS' in enabled_modules:
            from apps.bookings.models import Booking
        ...
        elif source_module == 'UTILITY_READINGS' and 'OPERATIONS' in enabled_modules:
            ...
            if tenant: qs = qs.filter(meter__tenant=tenant)
        ...
        elif source_module == 'FILM_FINANCE' and 'FINANCE' in enabled_modules:
            from apps.finance.models import DistributorShare
        ...
        elif source_module == 'CAFE_WASTAGE' and 'CAFE' in enabled_modules:
            from apps.revenue.models import CafeExpense
    ...
    @staticmethod
    def get_yearly_report(year: int, tenant=None) -> dict:
        """Aggregate monthly reports to produce a yearly financial summary."""
        months_data = []
        ticket_revenue = Decimal('0')
        canteen_gross = Decimal('0')
        cafe_expense = Decimal('0')
        ...
        film_advances = Decimal('0')
        payroll = Decimal('0')
        general_expenses_total = Decimal('0')
        general_exp_breakdown = {}
        dcr_gst = Decimal('0')
        dcr_etax = Decimal('0')
        dcr_cess = Decimal('0')
        dcr_repbeta = Decimal('0')
        dcr_kfc = Decimal('0')

        for month in range(1, 13):
            month_report = PLReportEngine.get_monthly_report(month, year, tenant=tenant)
            ticket_revenue += Decimal(str(month_report['income']['ticket_revenue']))
            canteen_gross += Decimal(str(month_report['income']['canteen_revenue']))
            ad_revenue += Decimal(str(month_report['income']['ad_revenue']))
            elec_charges += Decimal(str(month_report['expenses']['electricity']))
            diesel_cost += Decimal(str(month_report['expenses']['diesel']))
            distributor_cost += Decimal(str(month_report['expenses']['distributor_share']))
            film_advances += Decimal(str(month_report['expenses']['film_advances']))
            payroll += Decimal(str(month_report['expenses']['payroll']))
            cafe_expense += Decimal(str(month_report['expenses']['cafe_expenses']))
            general_expenses_total += Decimal(str(month_report['expenses']['general_expenses']))
            dcr_gst     += Decimal(str(month_report['expenses'].get('dcr_gst', 0)))
            dcr_etax    += Decimal(str(month_report['expenses'].get('dcr_etax', 0)))
            dcr_cess    += Decimal(str(month_report['expenses'].get('dcr_cess', 0)))
            dcr_repbeta += Decimal(str(month_report['expenses'].get('dcr_repbeta', 0)))
            dcr_kfc     += Decimal(str(month_report['expenses'].get('dcr_kfc', 0)))

            for k, v in month_report['expenses']['general_expenses_breakdown'].items():
                general_exp_breakdown[k] = general_exp_breakdown.get(k, 0.0) + float(v)

            months_data.append({
                'month': month,
                'income': month_report['income']['total'],
                'expenses': month_report['expenses']['total'],
                'net': month_report['net_profit'],
            })

        dcr_levies_total = dcr_gst + dcr_etax + dcr_cess + dcr_repbeta + dcr_kfc
        total_income = ticket_revenue + canteen_gross + ad_revenue
        total_expenses = elec_charges + diesel_cost + distributor_cost + film_advances + payroll + cafe_expense + general_expenses_total + dcr_levies_total
        net = total_income - total_expenses
