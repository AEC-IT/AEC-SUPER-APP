...
export const reportsAPI = {
  ...
  yearlyPL: (year) => api.get('/reports/pl/yearly/', { params: { year } }),
  alerts: (date) => api.get('/reports/alerts/', { params: { date } }),
  exportDailyCSV: (date) => `${import.meta.env.VITE_API_BASE || 'http://localhost:8002/api'}/reports/export/daily/?date=${date}`,
  exportMonthlyCSV: (month, year) => `${import.meta.env.VITE_API_BASE || 'http://localhost:8002/api'}/reports/export/monthly/?month=${month}&year=${year}`,
};
...
export const integrationsAPI = {
  ...
  dcrPost: (id) => api.post(`/integrations/dcr/${id}/post-to-finance/`),
  dcrReprocess: (id) => api.post(`/integrations/dcr/${id}/reprocess/`),
  dcrReviewMismatch: (id, note, resolved) => api.post(`/integrations/dcr/${id}/review-mismatch/`, { note, resolved }),
};
