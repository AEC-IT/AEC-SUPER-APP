...
export default function StaffReportPage() {
  ...
  const fetchStaff = async () => {
    setLoading(true);
    try {
      const response = await payrollAPI.staff();
      const mappedStaff = response.data.map(s => ({
        ...
        payrollStatus: s.payroll_status || 'READY',
        syncStatus: s.sync_status || 'SYNCHRONIZED',
  ...
  const handleCreateStaff = async (e) => {
    e.preventDefault();
    try {
      await payrollAPI.createStaff({
        employee_id: newStaff.staffId || `EMP-00${staffList.length + 1}`,
        name: newStaff.name,
  ...
  const handleSyncLogTrigger = async () => {
    toast.loading('Syncing with HR system...', { id: 'hr-sync' });
    try {
      const response = await payrollAPI.runSync();
      if (response.data?.sync_status === 'FAILED') {
        toast.error('Sync failed: ' + response.data.error_log, { id: 'hr-sync' });
      } else {
        toast.success(`Sync completed! Transferred ${response.data.records_received || 0} records.`, { id: 'hr-sync' });
      }
      fetchStaff();
    } catch (error) {
      toast.error('Failed to connect to HR App.', { id: 'hr-sync' });
    }
  };
      ...
      {activeTab === 'sync-log' && (
        <div className="card" style={{ padding: '24px' }}>
          <div className="flex-between mb-4">
            ...
            <button className="btn btn-secondary btn-sm" onClick={handleSyncLogTrigger} disabled={loading}>
              {loading ? 'Syncing...' : 'Trigger Manual Payroll Sync'}
            </button>
          </div>
          <table className="data-table">
            <thead>
            ...
            <tbody>
              {filteredStaff.map(s => (
                <tr key={s.staffId}>
                  ...
                  <td>
                    <span className={`badge ${s.syncStatus === 'OUT_OF_SYNC' || s.syncStatus === 'FAILED' ? 'badge-error' : 'badge-success'}`}>
                      {s.syncStatus}
                    </span>
                  </td>
