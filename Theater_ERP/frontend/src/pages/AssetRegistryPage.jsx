...
export default function AssetRegistryPage() {
  ...
  const [assetForm, setAssetForm] = useState({
    template: '', screen: '', serial_number: '', installed_date: TODAY,
    alert_threshold_hours: 100, is_active: true, counts_down: false,
    // inline template creation fields (used when no templates exist)
    new_manufacturer: '', new_model: '', new_rated_hours: 3000
  });
  ...
  const addAssetMutation = useMutation({
    mutationFn: async (d) => {
      let templateId = d.template;
      // If no existing template was selected, create one inline first
      if (!templateId && d.new_manufacturer && d.new_model) {
        const catObj = categoryList.find(c => c.key === categoryFilter);
        if (!catObj) throw new Error('Select a category first');
        const tmplRes = await operationsAPI.assetTemplates.create({
          category: catObj.id,
          manufacturer: d.new_manufacturer,
          model_number: d.new_model,
          rated_life_hours: d.new_rated_hours || null,
        });
        templateId = tmplRes.data.id;
        // Refresh templates list
        qc.invalidateQueries(['asset-templates', categoryFilter]);
      }
      if (!templateId) throw new Error('Please select or create a template');
      return operationsAPI.tenantAssets.create({
        template: templateId,
        screen: d.screen || null,
        serial_number: d.serial_number,
        installed_date: d.installed_date || null,
        alert_threshold_hours: d.alert_threshold_hours || null,
        is_active: d.is_active,
        counts_down: d.counts_down,
      });
    },
    onSuccess: () => {
      ...
      setAssetForm({ template: '', screen: '', serial_number: '', installed_date: TODAY, alert_threshold_hours: 100, is_active: true, counts_down: false, new_manufacturer: '', new_model: '', new_rated_hours: 3000 });
    },
    ...
    <div>
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          {activeTab === 'glasses' && (
            <button className="btn btn-primary" onClick={() => toast.success('Form overlay triggered')}>+ Manual Entry</button>
          )}
          {activeTab === 'tickets' && (
            ...
            <form onSubmit={e => { e.preventDefault(); addAssetMutation.mutate(assetForm); }}>
              <div className="grid-2">
                <div className="form-group">
                  ...
                  <select className="form-select" value={categoryFilter} onChange={e => { setCategoryFilter(e.target.value); setAssetForm(p => ({ ...p, template: '' })); }}>
              ...
              {/* Template selection or inline creation */}
              {templateList.length > 0 ? (
                <div className="form-group">
                  <label className="form-label">Model / Template</label>
                  <select className="form-select" value={assetForm.template} onChange={e => setAssetForm(p => ({ ...p, template: e.target.value }))} required={templateList.length > 0}>
                    <option value="">Select template…</option>
                    {templateList.map(t => <option key={t.id} value={t.id}>{t.manufacturer} {t.model_number} {t.rated_life_hours ? `(${t.rated_life_hours}h)` : ''}</option>)}
                  </select>
                </div>
              ) : (
                <>
                  <div style={{ background: 'rgba(251,191,36,0.08)', border: '1px dashed rgba(251,191,36,0.4)', borderRadius: 10, padding: '12px 16px', marginBottom: 16 }}>
                    <div style={{ fontSize: 12, color: 'var(--warning)', marginBottom: 8, fontWeight: 600 }}>⚡ No templates exist for this category yet — create one below:</div>
                    <div className="grid-2">
                      <div className="form-group" style={{ margin: 0 }}>
                        <label className="form-label">Manufacturer</label>
                        <input className="form-input" placeholder="e.g. Christie, Barco" value={assetForm.new_manufacturer} onChange={e => setAssetForm(p => ({ ...p, new_manufacturer: e.target.value }))} required />
                      </div>
                      <div className="form-group" style={{ margin: 0 }}>
                        <label className="form-label">Model Number</label>
                        <input className="form-input" placeholder="e.g. Xenon 3000H" value={assetForm.new_model} onChange={e => setAssetForm(p => ({ ...p, new_model: e.target.value }))} required />
                      </div>
                      <div className="form-group" style={{ margin: 0 }}>
                        <label className="form-label">Rated Life (hours)</label>
                        <input type="number" className="form-input" placeholder="3000" value={assetForm.new_rated_hours} onChange={e => setAssetForm(p => ({ ...p, new_rated_hours: e.target.value }))} />
                      </div>
                    </div>
                  </div>
                </>
              ...
              <div className="grid-2">
                <div className="form-group">
                  ...
                  <input className="form-input" placeholder="e.g. CHR-XEN-001" value={assetForm.serial_number} onChange={e => setAssetForm(p => ({ ...p, serial_number: e.target.value }))} required />
                </div>
                <div className="form-group">
                  <label className="form-label">Installed Date</label>
                  <input type="date" className="form-input" value={assetForm.installed_date} onChange={e => setAssetForm(p => ({ ...p, installed_date: e.target.value }))} />
                </div>
                <div className="form-group">
                  <label className="form-label">Alert Threshold (hours remaining)</label>
                  <input type="number" className="form-input" value={assetForm.alert_threshold_hours} onChange={e => setAssetForm(p => ({ ...p, alert_threshold_hours: e.target.value }))} />
                </div>
                <div className="form-group">
                  <label className="form-label">Reading Direction</label>
                  <select className="form-select" value={assetForm.counts_down ? 'down' : 'up'} onChange={e => setAssetForm(p => ({ ...p, counts_down: e.target.value === 'down' }))}>
                    <option value="up">⬆️ Counts Up (1 → 2 → 3…)</option>
                    <option value="down">⬇️ Counts Down (3000 → 2999…)</option>
                  </select>
                </div>
              </div>

              {assetForm.counts_down && (
                <div style={{ background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 8, padding: '8px 14px', fontSize: 12, color: 'var(--text-secondary)', marginBottom: 12 }}>
                  💡 <strong>Countdown mode:</strong> Opening value will be higher than closing (e.g. 2999 → 2998). The system automatically calculates consumption as the absolute difference.
                </div>
              )}
              ...
              <div style={{ textAlign: 'right', marginTop: 16, display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={addAssetMutation.isLoading}>
                  {addAssetMutation.isLoading ? '⏳ Registering…' : '✅ Register Asset'}
                </button>
              </div>
            </form>
          </div>
